...
| Formal title | `products.formal_title`, fallback `name` | `PRODUCT_SELECT`; `mapRow`: `formal_title ?? name` | Metadata title fallback | Product page / `product:<slug>` | Yes | Yes | Import and presentation refresh |
| Subtitle | `products.subtitle`, fallback `tagline` | `PRODUCT_SELECT`; `mapRow` fallback | Domain value; not a dedicated current PDP block | Product page / `product:<slug>` | Yes | Yes | Presentation refresh |
| Card tagline | `products.card_tagline`, fallback `tagline` | `PRODUCT_SELECT`; `mapRow` fallback | Hero tagline, metadata description fallback, discovery | Product page / `product:<slug>` | Yes | Yes | Import and presentation refresh |
| Description | `editorial_description`, fallback `description` | `PRODUCT_SELECT`; `mapRow`; `ProductDetail.compactDescription` | Hero description and deeper fallback copy | Product page / `product:<slug>` | Yes | Yes | Import and presentation refresh |
