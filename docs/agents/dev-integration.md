...
`pnpm github:workflow:plan` audits the labels, classic protections, default Actions token permissions, GitHub Actions integration identity, and the `dev Integration Line authority` ruleset without changing GitHub. The proposed cutover sets the repository default token to read-only and forbids token-authored review approval. Every non-coordinator workflow is explicitly read-only except `.github/workflows/scheduled-browser-verification.yml`, whose separately audited authority is limited to `issues: write` for the single deduplicated operational verification issue; it has no contents, pull-request, Actions, or Integration Line write authority. The ruleset restricts `dev` updates to the GitHub Actions Integration through pull requests and binds `ci` and `dev-integration` to that Integration ID. The coordinator is the only trusted workflow in the audited tree that requests repository integration write permissions. `dev` does not require an up-to-date head because the coordinator freezes and verifies the exact current base; `main` remains strict.
