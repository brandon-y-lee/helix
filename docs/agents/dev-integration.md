...
subprocess receives the repository-scoped fine-grained token stored as
`INTEGRATION_MERGE_TOKEN`, injected under `GH_TOKEN`; the dedicated secret name
is stripped from every child environment. That user-token-authored merge emits the protected
