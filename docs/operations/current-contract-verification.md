...
The Management API's `read_only: true` selects `supabase_read_only_user`, which intentionally lacks permission to execute the protected Catalog document reader. Capture therefore selects the authorized role with `read_only: false` and encloses each fixed read in `BEGIN READ ONLY; SET LOCAL TIME ZONE 'UTC'; …; ROLLBACK;`. The evidence must still report an actual read-only transaction and complete RLS-bypassing inventory; grants are never widened. A SQL error can skip the trailing rollback: capture fails without an artifact. The [official query route](https://github.com/supabase/postgres-meta/blob/f380cc5be21edef4e77d9838c732d1f20af0b3c0/src/server/routes/query.ts) invokes per-request pool cleanup on both success and SQL error; the [connection implementation](https://github.com/supabase/postgres-meta/blob/f380cc5be21edef4e77d9838c732d1f20af0b3c0/src/lib/db.ts) initiates shutdown. Do not treat a failed request as a successful capture, an explicit rollback receipt, or proof that shutdown completed before the HTTP error.

Complete evidence is returned as SQL `text`, because [node-postgres parses JSONB automatically](https://node-postgres.com/features/types) and could round precise Source numbers before an API response reaches the CLI. The CLI requires the untouched JSON text and refuses numbers it cannot preserve exactly; it never certifies hashes over silently rounded facts.
