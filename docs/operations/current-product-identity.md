...
5. With exact target/apply authority and the manifest review complete, open a privileged SQL session to the verified project. Start with `BEGIN ISOLATION LEVEL READ COMMITTED`, then set `lock_timeout = '5s'`, `statement_timeout = '30s'`, and `idle_in_transaction_session_timeout = '60s'` locally. Do not change isolation after a snapshot has been established. Load `supabase/operations/upgrade_current_treat_identity.sql`, then call `pg_temp.upgrade_current_treat_identity(reviewed_document, reviewed_revision, administrator_id)`. Run the already-reviewed postflight assertions and commit promptly; do not pause for human review while the write fence is held. The temporary function is session-local and has no application RPC or browser grants. A failure aborts the transaction; rollback and resolve the changed state instead of weakening a guard.
...
The runner also hashes and executes `supabase/tests/catalog_identity_isolation.mjs`. Its two-session proof establishes a reviewed no-Draft snapshot, commits the real normal Create Draft in a second session, and then invokes the identity operation in the first. Explicit SQL output milestones synchronize those steps. Both predecessor identities and already-current Super Serum refuse stale Repeatable Read with `25001`; paired Read Committed cases observe the Draft and refuse with `23514`. Fresh connections compare all 14 checkpoint Catalog tables to the state after Create Draft committed, preserving its Draft/audit, canonical and child rows, and immutable history. Direct unsupported-isolation calls also verify isolation refusal precedes input validation. Existing ordinary publication, no-op, retry and independent writer proofs remain required.
