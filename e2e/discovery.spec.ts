...
test("collection chip deep-links into the shop, pre-filtered", async ({
  ...
  await page.goto("/");
  await page.getByRole("link", { name: "Treat", exact: true }).click();

  await expect(page).toHaveURL(/\/products\?collection=Treat$/);
  // Treat has 2 products in the seed catalog.
...
test("PDP complete-the-routine renders and a related product navigates", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Complete the routine" }),
  ).toBeVisible();
