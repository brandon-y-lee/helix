-- Complete the ticket #184 operational contract with the transactional
-- adjustment path needed by refund processing.

create function public.record_rewards_points_adjustment(
  p_user_id uuid,
  p_points integer,
...
begin
  ...
  if found then
    if v_existing.user_id is distinct from p_user_id
      ...
      or v_existing.metadata is distinct from pg_catalog.coalesce(p_metadata, '{}'::jsonb)
    then
  ...
  if v_balance + p_points < 0 then
    raise exception using errcode = 'P0001', message = 'Insufficient rewards balance';
  end if;
...
revoke all on function public.record_rewards_points_adjustment(uuid, integer, public.loyalty_ledger_entry_type, text, text, uuid, jsonb) from public, anon, authenticated, service_role;
grant execute on function public.record_rewards_points_adjustment(uuid, integer, public.loyalty_ledger_entry_type, text, text, uuid, jsonb) to service_role;

comment on function public.record_rewards_points_adjustment(uuid, integer, public.loyalty_ledger_entry_type, text, text, uuid, jsonb) is
  'Temporary ticket #184 transactional operational adjustment interface; remove in ticket #187.';
